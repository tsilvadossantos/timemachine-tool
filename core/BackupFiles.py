...
class BackupFiles(DataModel):
    ...
    def execute_backup(self, backup_dest):
        ...
        if new_modified_date_dict:
            yaml_dump_list = self.generate_temp_data_model(new_modified_date_dict, self.dm.get_file_name_dict(), self.dm.get_description_dict())

            #wriet to file
            self.f.write_to_yam_file(yaml_dump_list)
